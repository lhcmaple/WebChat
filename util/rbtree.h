...
data * rb_search(rb_root *root,int val)
{
	...
	while (n)
	{
		page = rb_entry(n, data, node);

		if (val < page->val)
			n = n->rb_left;
		else if (val>page->val)
			n = n->rb_right;
		else
			return page;
	}
...
data * __rb_insert(rb_root *root,int val,rb_node *node)
{
	...
	{
		parent = *p;
		page = rb_entry(parent, data, node);
...
data * rb_insert(rb_root *root,int val,rb_node *node)
{
    data *ret;
